...
async function loadIconifyIconImage(icon: IconifyIcon, color: string, size: number) {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      image.onerror = () => reject(new Error("Impossible de charger l'icône du badge."));
      image.src = url;
    });
    return image;
  } finally {
...
export async function loadBadgeCertifiedIcon(size: number) {
  return loadIconifyIconImage(BADGE_CERTIFIED_ICON, BADGE_CERTIFIED_COLOR, size);
}
